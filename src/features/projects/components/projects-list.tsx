...
const ProjectItem = ({ data }: {
    ...
    return (
        <Link href={`/projects/${data._id}`}
            className="text-sm text-foreground/60 font-medium hover:text-foreground py-1 flex items-center justify-between w-full grpup">
            <div className="flex items-center gap-2">
                {getProjectIcon(data)}
...
export const ProjectsList = ({
    ...
    if (projects === undefined) {
        return <Spinner className="size-6 text-ring" />
    }
